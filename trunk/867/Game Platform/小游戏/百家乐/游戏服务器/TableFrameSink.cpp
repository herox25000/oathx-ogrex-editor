	...
	case GER_NORMAL:		//常规结束
		{
			...
			//发送积分
			GameEnd.cbTimeLeave=TIME_FREE;	//剩余时间
			if ( m_CurrentBanker.dwUserID != 0 ) 
				GameEnd.lBankerTreasure = m_CurrentBanker.lUserScore;//庄家金币
			...
			_snprintf(szPokerInfo, CountArray(szPokerInfo), TEXT("庄:%d张 闲:%d张 庄_%X_%X_%X 闲_%X_%X_%X"), 
			...
			_snprintf(szPointInfo, CountArray(szPointInfo), TEXT("庄:%d点 闲:%d点"), cbBankerCount, cbPlayerCount);
			//写入回合信息
	...
	case GS_FREE + 1:
		{
			...
			//全局信息
			DWORD dwPassTime=(DWORD)time(NULL)-m_dwJettonTime;
			StatusFree.cbTimeLeave=(BYTE)(TIME_PLACE_JETTON-__min(dwPassTime,TIME_FREE));
			//发送场景
			bool bSuccess = m_pITableFrame->SendGameScene(pIServerUserItem,&StatusFree,sizeof(StatusFree));
			//发送申请者
			SendApplyUser(  pIServerUserItem );
...
void CTableFrameSink::CalculateScore()
{
	...
	//if(AllUserWinScore> (MypUserScore->lScore + MyBankerWinScore))//如果庄家手中的银子加赢来的银子还不够陪付
...
{
	_snprintf(szJetton, MAX_DB_JETTON_LEN, TEXT("椅:%d 平:%I64d 庄:%I64d 闲:%I64d 平王:%I64d 庄王:%I64d 闲王:%I64d"),
		wChairID,
