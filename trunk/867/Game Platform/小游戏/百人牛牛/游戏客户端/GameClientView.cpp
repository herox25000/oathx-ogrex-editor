...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	//列表控件
	DeferWindowPos(hDwp,m_ApplyUser,NULL,LifeWidth+245+20,TopHeight+10+10,310,88,uFlags);
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (INT i=0;i<AREA_COUNT;i++)
	{
		//变量定义
		LONG lScoreCount=0L;
		LONG static lScoreJetton[JETTON_COUNT]={100L,1000L,10000L,50000L,100000L,500000L,1000000L,5000000L};
...
void CGameClientView::PlaceUserJetton(BYTE cbViewIndex, __int64 lScoreCount)
{
	...
	LONG lScoreIndex[JETTON_COUNT]={100L,1000L,10000L,50000L,100000L,500000L,1000000L,5000000L};
...
void CGameClientView::DeduceWinner(bool &bWinTian, bool &bWinDi, bool &bWinXuan,bool &bWinHuan,BYTE &TianMultiple,BYTE &diMultiple,BYTE &TianXuanltiple,BYTE &HuangMultiple )
{
  ...
  /*  for(int i = 0;i<5;i++) 
	{
		BYTE bcTmp[5];
		m_lUserCardType[i] = m_GameLogic.GetCardType(m_cbTableCardArray[i],5,bcTmp);
		int iType = m_lUserCardType[i];
	
		if(iType==CT_POINT)
		{
			CopyMemory(m_cbTableSortCardArray[i],bcTmp,5);
		}else
		{
			CopyMemory(m_cbTableSortCardArray[i],bcTmp+3,2);
			CopyMemory(m_cbTableSortCardArray[i]+2,bcTmp,3);
