...
bool CGameClientDlg::DispatchUserCard(BYTE cbAnimalBox)
{
	...
	m_lTotalTime=cbAnimalBox*ANIMAL_ROLL_SPEED;
	...
	case IDI_PLAY_ANIMAL:		//派发扑克
		{
			...
			if ( dwSub<half )
			{
				cbNew=(dwSub*dwSub*dwSub)*(m_GameClientView.m_cbAnimalBox/2)/(half*half*half);
			}
			else if ( dwSub>=half && dwSub<=m_lTotalTime)
			{
				__int64 unOver=(m_lTotalTime-dwSub);
				cbNew=m_GameClientView.m_cbAnimalBox;
				cbNew-=(unOver*unOver*unOver)*(m_GameClientView.m_cbAnimalBox/2)/(half*half*half);
			}
			else
				cbNew=m_GameClientView.m_cbAnimalBox;
