...
{
	//位置信息
	m_nWinFlagsExcursionX = nWidth/2-263;	
	...
	//列表控件
	DeferWindowPos(hDwp,m_ApplyUser,NULL,nWidth/2 + 150,nHeight/2-294,220,52,uFlags);
	DeferWindowPos(hDwp,m_btUp,NULL,nWidth/2 + 150 + 204, nHeight/2-294,75/5,16,uFlags);
	DeferWindowPos(hDwp,m_btDown,NULL,nWidth/2 + 150 + 204, nHeight/2-294 + 36,75/5,16,uFlags);
	if(1)
	{
	...
	//筹码按钮
	CRect rcJetton;
	int nXPos = nWidth / 2 - 205;
	int nYPos = nHeight / 2 + 176;
	for (int i=0; i<7; i++)
	{
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (INT i=0;i<6;i++)
	{
		...
		uAllScoreCount+=(lScoreCount-m_nJettonScoreDec[i]);
	}
	...
	if ( m_bstate == Status_DisCard ) 
		...
		m_ImageTimeFlag.AlphaDrawImage(pDC, nWidth/2-160, nHeight/2-270, nTimeFlagWidth, m_ImageTimeFlag.GetHeight(), 0, 0,RGB(255,0,255));
...
void CGameClientView::SetGameHistory(enOperateResult OperateResult, BYTE cbPlayerCount, BYTE cbBankerCount,WORD wWinnerSide)
{
	...
	if ( 15 < nHistoryCount ) m_btScoreMoveR.EnableWindow(TRUE);

	//移到最新记录
	if ( 15 < nHistoryCount )
	{
		m_nScoreHead = (m_nRecordLast - 15 + MAX_SCORE_HISTORY) % MAX_SCORE_HISTORY;
...
{
	//效验参数
	ASSERT(cbViewIndex<=ID_QIAO);
	if (cbViewIndex>ID_QIAO) return;
...
void CGameClientView::DrawWinFlags(CDC * pDC)
{
	...
	CString strPoint;
	int nDrawCount = 0;
	while ( nIndex != m_nRecordLast && ( m_nRecordLast!=m_nRecordFirst ) && nDrawCount < 15 )
	{
		...
		if(ClientGameRecord.enOperateFlags == enOperateResult_NULL)
		{
			...
			for(WORD i=ID_SHUN_MEN; i<=ID_DAO_MEN ;i*=2)
			{
				...
				{
					m_ImageWinFlags.AlphaDrawImage( pDC, nXPos, nYPos+32*nCount, m_ImageWinFlags.GetWidth()/4 , 
						m_ImageWinFlags.GetHeight(),0/*m_ImageWinFlags.GetWidth()/4*/, 0, RGB(255, 0, 255) );
				}
				else
				{
					m_ImageWinFlags.AlphaDrawImage( pDC, nXPos, nYPos+32*nCount, m_ImageWinFlags.GetWidth()/4 , 
						m_ImageWinFlags.GetHeight(),m_ImageWinFlags.GetWidth()/4 * 2, 0, RGB(255, 0, 255) );
				}
				nCount++;
			}
		}
		else
		{
			if(ClientGameRecord.wWinner & ID_SHUN_MEN)
			{
				if(ClientGameRecord.lMeShunMenScore>0)
				{
					m_ImageWinFlags.AlphaDrawImage( pDC, nXPos, nYPos, m_ImageWinFlags.GetWidth()/4 , 
						m_ImageWinFlags.GetHeight(),m_ImageWinFlags.GetWidth()/4 , 0, RGB(255, 0, 255) );
				}
			...
			else
			{
				...
				{
					m_ImageWinFlags.AlphaDrawImage( pDC, nXPos, nYPos, m_ImageWinFlags.GetWidth()/4 , 
						m_ImageWinFlags.GetHeight(),m_ImageWinFlags.GetWidth()/4 * 3, 0, RGB(255, 0, 255) );
				}
				else if(ClientGameRecord.lMeZuoJiaoScore>0 && !(ClientGameRecord.wWinner & ID_TIAN_MEN) ||	\
					ClientGameRecord.lMeQiaoScore>0 && !(ClientGameRecord.wWinner & ID_DAO_MEN))
				{
			...
			if(ClientGameRecord.wWinner & ID_DAO_MEN)
			{
				...
				{
					m_ImageWinFlags.AlphaDrawImage( pDC, nXPos, nYPos+32, m_ImageWinFlags.GetWidth()/4 , 
						m_ImageWinFlags.GetHeight(),m_ImageWinFlags.GetWidth()/4 , 0, RGB(255, 0, 255) );
				}
			...
			if(ClientGameRecord.wWinner & ID_TIAN_MEN)
			{
				...
				{
					m_ImageWinFlags.AlphaDrawImage( pDC, nXPos, nYPos+32*2, m_ImageWinFlags.GetWidth()/4 , 
						m_ImageWinFlags.GetHeight(),m_ImageWinFlags.GetWidth()/4 , 0, RGB(255, 0, 255) );
				}
...
__int64 CGameClientView::GetMaxPlayerScore()
{
	...
	if(pBankData == NULL)
		return 0;
	//最大下注
	__int64 lMaxPlayerScore =pBankData->lScore-lAllAreaScore;
...
void CGameClientView::RemoveUserJetton( BYTE cbViewIndex, __int64 lScoreCount )
{
	...
	m_nJettonScoreDec[nIndex - 1] = lScoreCount;
	//更新界面
	UpdateGameView(NULL);
	return;
}
