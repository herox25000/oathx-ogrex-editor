...
namespace O2
{
	...
	BYTE		SmallNineAndroid::GetRandArea()
	{
		// 获取庄配置
		SAppConfig* pConfig = ConfigFile::GetSingleton().GetAppConfig();
		int nTotal = pConfig->wShunMenRate + pConfig->wTianMenRate + pConfig->wDaoMenRate + pConfig->wZuoJiaoRate + pConfig->wYouJiaoRate + pConfig->wQiaoRate;
		int nShunRate = pConfig->wShunMenRate * 100 / nTotal;
		int nTianRate = pConfig->wTianMenRate * 100 / nTotal;
		int nDaoRate = pConfig->wDaoMenRate * 100 / nTotal;
		int nZuoJiao = pConfig->wZuoJiaoRate * 100 / nTotal;
		int nYouJiao = pConfig->wYouJiaoRate * 100 / nTotal;
		int nQiao = pConfig->wQiaoRate * 100 / nTotal;
		// 随机压注门
		static BYTE cbArea[] = {
				ID_SHUN_MEN,
				ID_TIAN_MEN,
