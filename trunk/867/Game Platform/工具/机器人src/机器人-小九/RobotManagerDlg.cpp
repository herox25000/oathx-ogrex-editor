...
void CRobotManagerDlg::ChangeRobot()
{
	...
	::GetPrivateProfileString("config", "ip", "", 
		m_strIP.GetBuffer(255), 255, CString(path)+"\\RM-XiaoJiu.ini");
	m_strIP.ReleaseBuffer();

	if(m_strIP.IsEmpty())
	{
		MessageBox("配置文件:RM-XiaoJiu.ini丢失，无法启动！");
		return;
	}

	m_nPort = ::GetPrivateProfileInt("config", "port", 0, 
		CString(path)+"\\RM-XiaoJiu.ini");

	::GetPrivateProfileString("config", "psw", "", 
		m_strPsw.GetBuffer(255), 255, CString(path)+"\\RM-XiaoJiu.ini");
