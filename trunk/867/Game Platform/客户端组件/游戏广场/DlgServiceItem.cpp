...
BOOL CDlgServiceItem::OnEraseBkgnd(CDC * pDC)
{
	...
	//填充背景
	pDC->FillSolidRect(&rcClient,CSkinPngDialog::m_SkinAttribute.m_crBackGround);
	...
	case CTLCOLOR_STATIC:
		{
			...
			pDC->SetBkColor(CSkinPngDialog::m_SkinAttribute.m_crBackGround);
			pDC->SetTextColor(CSkinPngDialog::m_SkinAttribute.m_crControlText);
			return CSkinPngDialog::m_SkinAttribute.m_brBackGround;
		}
	}
...
BOOL CDlgServiceBrowser::OnInitDialog()
{
	...
	CRect rcClient;
	GetClientRect(&rcClient);
	TCHAR szPlatformLink[]=TEXT("www.game541.com");				//平台网站
	//创建控件
