...
//! 初始化 [10/14/2012 bbsssssss]
CString CSkinImage::ms_BasePath = "";
CImageMap* CSkinImage::ms_pImageMap = NULL;
...
int CSkinImage::LockResource()
{
	if (m_nRefCount==0)
	{
		...
		{
			LoadImageFromFile(ms_BasePath + m_LoadInfo.strFileName);
		}
		else
		{
			//! 如果在列表里有该Resource的关联信息..则使用该信息进行资源加载 [11/7/2012 bbsssssss]
			CString strResPath;
			if(CSkinImage::ms_pImageMap && CSkinImage::ms_pImageMap->GetImageInfo(m_LoadInfo.uResourceID, strResPath))
			{
				LoadImageFromFile(strResPath);
			}
			else
			{
...
HBITMAP CSkinImage::LoadImageFromFile(PCTSTR pszBitmapFile)
{
	IPicture* pIPic;
	...
	void* pVoid;
	FILE *fp = NULL;
	fp = fopen(CT2A( pszBitmapFile ),"rb");
	if ( fp == NULL )
	{
		OutputDebugStringA(CT2A( pszBitmapFile ));
