...
bool __cdecl CAttemperEngineSink::DeleteUserItem(IServerUserItem * pIServerUserItem)
{
	...
	pIServerUserItem->GetUserModifyScoreInfo(LeaveGameServer.ScoreModifyInfo);//
...
//请求事件
bool __cdecl CAttemperEngineSink::PostDataBaseRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	return m_pIDataBaseEngine->PostDataBaseRequest(wRequestID,dwContextID,pData,wDataSize);
}
	...
	case SUB_GR_USER_LOOKON_REQ:	//旁观桌子
		{
			...
			//在玩家选择座位和服务器随机分配座位的情况下如果用户禁止了旁观  玩家不能坐下
			if(m_pGameServiceOption->cbDistributeMode==DISTRIBUTE_MODE_NONE
				|| m_pGameServiceOption->cbDistributeMode==DISTRIBUTE_MODE_RAND)
			{
				CTableFrame * pTableFrame=(m_pTableFrame+pUserSitReq->wTableID);
				if(pTableFrame->m_bAllowLookon[pUserSitReq->wChairID] )
				{
					CMD_GR_SitFailed SitFailed;
					memset(&SitFailed,0,sizeof(SitFailed));
					_snprintf(SitFailed.szFailedDescribe,sizeof(SitFailed.szFailedDescribe), TEXT("对不起，此用户已经禁止被旁观"));
					WORD wSendSize=sizeof(SitFailed)-sizeof(SitFailed.szFailedDescribe)+CountStringBuffer(SitFailed.szFailedDescribe);
					SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_SIT_FAILED,&SitFailed,wSendSize);
					return true;
				}
			...
			else if ( m_pGameServiceOption->cbDistributeMode==DISTRIBUTE_MODE_RAND )
			{
				WORD wChairID=INVALID_CHAIR;

				CTableFrame* pTableFrame=NULL;
				WORD wTableCount=m_pGameServiceOption->wTableCount;
				for ( WORD i=0; i<wTableCount; i++ )
				{
					...
					WORD temp=pTableFrame->GetNullChairID();
					if ( temp!=INVALID_CHAIR )
					{
						wChairID=temp;
						break;
					}
				...
				if ( pTableFrame && (wChairID!=INVALID_CHAIR) )
				{
					pTableFrame->SendGameMessage(pIServerUserItem, "此游戏房间是随机安排座位的, 服务器把您自动安排到另外的桌子...", SMT_INFO);

					if ( wSubCmdID==SUB_GR_USER_SIT_REQ )	
						pTableFrame->PerformSitDownAction(wChairID, pIServerUserItem,pUserSitReq->szTablePass);
					else
						pTableFrame->PerformLookonAction(wChairID, pIServerUserItem,pUserSitReq->szTablePass);
				}
			}
			else if ( m_pGameServiceOption->cbDistributeMode==DISTRIBUTE_MODE_NO_LOOK )
			{
				//坐下位置
				CTableFrame * pTableFrame=(m_pTableFrame+pUserSitReq->wTableID);
				WORD wChairID=pTableFrame->GetNullChairID();
...
void CAttemperEngineSink::ModifyGameGold(IServerUserItem * pIServerUserItem,LONG lScore)
{
	...
	if(m_pGameServiceOption->wServerType == GAME_GENRE_GOLD)
	{
		...
		{
			__int64 lUserScore=pServerUserData->UserScoreInfo.lScore;
...
//转账记录查询完成
bool CAttemperEngineSink::OnDBTransferLogOver(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//发送消息
	CMD_GF_Transfer_Log_Out* pTransferLog=(CMD_GF_Transfer_Log_Out*)pData;
	IServerUserItem * pIServerUserItem=m_ServerUserManager.SearchOnLineUser(pTransferLog->dwUserID);
	if ( pIServerUserItem )
	{
		this->SendData(pIServerUserItem, MDM_GF_FRAME, SUB_GF_TRANSFER_LOG, pTransferLog, sizeof(CMD_GF_Transfer_Log_Out));
	}
	return true;
}

//转账记录返回
bool CAttemperEngineSink::OnDBTransferLogItem(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//发送消息
	CMD_GF_Transfer_Log_Item* pLogItem=(CMD_GF_Transfer_Log_Item*)pData;
	IServerUserItem * pIServerUserItem=m_ServerUserManager.SearchOnLineUser(pLogItem->dwUserID);
	if ( pIServerUserItem )
	{
		this->SendData(pIServerUserItem, MDM_GF_FRAME, SUB_GF_TRANSFER_LOG_ITEM, pLogItem, sizeof(CMD_GF_Transfer_Log_Item));
	}
	return true;
}
