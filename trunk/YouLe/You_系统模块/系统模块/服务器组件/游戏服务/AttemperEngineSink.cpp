...
bool CAttemperEngineSink::OnEventBankDrawoutGold(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
		if(pServerUserData->wTableID!=INVALID_TABLE && (m_pTableFrame+pServerUserData->wTableID)->IsUserPlaying(pIServerUserItem))
		{
			SendGameMessage(dwSocketID,TEXT("请结束游戏后，再进行提取操作!"),SMT_EJECT);
			SendRoomMessage(dwSocketID,TEXT("请结束游戏后，再进行提取操作!"),SMT_EJECT);
			return true;
		}
	}
...
bool CAttemperEngineSink::OnEventBankStorage(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
	{
		if(pServerUserData->wTableID!=INVALID_TABLE && (m_pTableFrame+pServerUserData->wTableID)->IsUserPlaying(pIServerUserItem))
		{
			SendRoomMessage(dwSocketID,TEXT("请结束游戏后，再进行存储操作!"),SMT_EJECT);
			return true;
		}
	}
