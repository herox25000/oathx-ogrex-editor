		...
		//else
		{
			...
			if (iErrorCode==SOCKET_ERROR)
			{
				...
				if (iErrorCode!=WSAEWOULDBLOCK)
				{
					...
					_sntprintf(szBuffer,CountArray(szBuffer),TEXT("连接发生错误，错误代码 [ %d ]"),iErrorCode);
...
#ifdef _UNICODE
	USES_CONVERSION;
	char*   lpszT   =   W2A(pszServerAddr); 
	//转化地址
	DWORD dwServerIP=inet_addr(lpszT);//pszServerAddr);

	if (dwServerIP==INADDR_NONE)
	{
		LPHOSTENT lpHost=gethostbyname(lpszT);//pszServerAddr);
...
LRESULT CTCPSocket::OnSocketNotifyClose(WPARAM wParam, LPARAM lParam)
{
	...
	//if (iErrorCode==0) m_cbSocketStatus=SOCKET_STATUS_CONNECT;
	//else CloseSocket();
	m_pITCPSocketSink->OnEventTCPSocketShut(m_wSocketID,iErrorCode);
