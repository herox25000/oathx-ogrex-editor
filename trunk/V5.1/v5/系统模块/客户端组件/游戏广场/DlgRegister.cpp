...
CDlgRegister::CDlgRegister() : CDialog(IDD_DLG_REGISTER)
{
	...
	//创建画刷
	m_brBrush.CreateSolidBrush(RGB(211,223,231));
...
BOOL CDlgRegister::OnInitDialog()
{
	...
	m_ChangeValidate.SetBackGroundColor(RGB(211,223,231));
	...
	m_btQuit.SetButtonImage(IDB_BT_REGISTER_QUIT,hInstance,false,false);
	m_btEnter.SetButtonImage(IDB_BT_REGISTER_ENTER,hInstance,false,false);
	...
	if(pGlobalUnits!=NULL && pGlobalUnits->LocalFileExist(szBillUrl))
	{
		...
		m_PlatformPublicize.SetWindowPos(NULL,50,9,SizeWindow.cx-101,18,SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOACTIVATE);
	}
	else
		m_PlatformPublicize.SetWindowPos(NULL,50,9,SizeWindow.cx-101,18,SWP_HIDEWINDOW|SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOACTIVATE);
...
bool CDlgRegister::GetInformation()
{
	...
	GetControlString(IDC_NICK_NAME,m_szNickName,CountArray(m_szNickName));
...
BOOL CDlgRegister::OnEraseBkgnd(CDC * pDC)
{
	...
	//绘画等级
	ImagePassword.DrawImage(pBufferDC,249,276,SizePassword.cx*3,SizePassword.cy,0,0);
	ImagePassword.DrawImage(pBufferDC,249,332,SizePassword.cx*3,SizePassword.cy,0,0);

	//绘画叠加
	if (m_cbLogonPassLevel>=PASSWORD_LEVEL_1)
	{
		INT nImagePos=(m_cbLogonPassLevel-PASSWORD_LEVEL_1)*SizePassword.cx;
		ImagePassword.DrawImage(pBufferDC,249+nImagePos,276,SizePassword.cx,SizePassword.cy,nImagePos,SizePassword.cy);
	}
	...
	if (m_cbInsurePassLevel>=PASSWORD_LEVEL_1)
	{
		...
		ImagePassword.DrawImage(pBufferDC,249+nImagePos,332,SizePassword.cx,SizePassword.cy,nImagePos,SizePassword.cy);
	}
