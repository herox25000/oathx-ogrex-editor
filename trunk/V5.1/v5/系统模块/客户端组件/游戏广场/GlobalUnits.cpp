...
//文件判断
bool CGlobalUnits::LocalFileExist(LPCTSTR pszFileName)
{
	//变量定义
	CFile  file; 

	//打开文件
	bool bFileExist = file.Open(pszFileName ,CFile::modeRead)==TRUE;
	
	//关闭文件
	if(bFileExist) file.Close();

	return bFileExist;
}

//设置参数
void CGlobalUnits::SetValidateParam(DWORD dwUserID,LPCTSTR szPassword)
{
	//_sntprintf(m_szValidateParam,CountArray(m_szValidateParam),szValidateLink,dwUserID,szPassword);

	return;
}

//获取参数
LPCTSTR CGlobalUnits::GetValidateParamAsString()
{
	return m_szValidateParam;
}

//播放声音
bool CGlobalUnits::PlayPlazaSound(LPCTSTR pszSoundName)
{
	//播放判断
...
//播放声音
bool CGlobalUnits::PlayPlazaSound(HINSTANCE hInstance, LPCTSTR pszSoundName)
{
	//播放判断
	if (AfxGetMainWnd() == NULL) return false;
