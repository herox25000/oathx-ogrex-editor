...
//绘画函数
VOID CTableView::DrawTableView(CDC * pDC,INT nXTablePos,INT nYTablePos,DWORD dwServerRule, CTableResource * pTableResource)
{
	//椅子大小
	...
	//绘制桌子
	if (pTableResource->m_bShowTable==true)
	{
		if (pTableResource->m_ImageTable.IsNull()==false)
		{
			//填充背景
			pTableResource->m_ImageBackSurface.BitBlt(*pDC,0,0,pTableResource->m_SizeTable.cx,pTableResource->m_SizeTable.cy,nXTablePos,nYTablePos);

			//构造桌子
			INT nXPos=(m_TableAttribute.bPlaying==true)?SizeTable.cx:0;
			pTableResource->m_ImageTable.DrawImage(pDC,0,0,pTableResource->m_SizeTable.cx,pTableResource->m_SizeTable.cy,nXPos,0);
		}
	}
		...
		for (WORD i=0;i<m_TableAttribute.wChairCount;i++)
		{
			...
			if (pTableResource->m_bShowUser==true)
			{
				...
				if ((bAvertCheatMode==false)&&(m_TableAttribute.pIClientUserItem[i]!=NULL))
				{
					...
					{
						//自定头像
						INT nXImagePos=(i*rcChair.Width()>=pTableResource->m_ImageCustomUser.GetWidth())?0:i*rcChair.Width();
...
VOID CTableViewFrame::OnPaint()
{
	...
	//调整尺寸
	if ((rcClient.Width()>m_TableResource.m_SizeBackSurface.cx)||(rcClient.Height()>m_TableResource.m_SizeBackSurface.cy))
	{
		//设置大小
		m_TableResource.m_SizeBackSurface.cx=__max(m_TableResource.m_SizeBackSurface.cx,rcClient.Width());
		m_TableResource.m_SizeBackSurface.cy=__max(m_TableResource.m_SizeBackSurface.cy,rcClient.Height())+m_TableResource.m_SizeTable.cy;

		//销毁缓冲
		if (m_TableResource.m_ImageBackSurface.IsNull()==false) m_TableResource.m_ImageBackSurface.Destroy();

		//创建缓冲
		m_TableResource.m_ImageBackSurface.Create(m_TableResource.m_SizeBackSurface.cx,m_TableResource.m_SizeBackSurface.cy,32);

		//创建 DC
		CImageDC BufferDC(m_TableResource.m_ImageBackSurface);
		CDC * pDCBuffer=CDC::FromHandle(BufferDC);
		
		//绘制背景
		for(INT nHeight=0;nHeight<m_TableResource.m_SizeBackSurface.cy;nHeight+=m_TableResource.m_ImageGround.GetHeight())
		{
			for(INT nWidth=0;nWidth<m_TableResource.m_SizeBackSurface.cx;nWidth+=m_TableResource.m_ImageGround.GetWidth())
			{
				m_TableResource.m_ImageGround.BitBlt(*pDCBuffer,nWidth,nHeight);
			}
		}
	}
	...
	if ((m_TableViewArray.GetCount()>0L)&&(m_TableResource.m_bShowTable==true))
	{
		...
		for (INT nYIndex=nYStartItem;nYIndex<nYConcludeItem;nYIndex++)
		{
			for (INT nXIndex=nXStartItem;nXIndex<nXConcludeItem;nXIndex++)
			{
				...
				{
					//绘画桌子
					m_TableViewArray[nIndex]->DrawTableView(pDCBuffer,nXTablePos,nYTablePos,m_dwServerRule,&m_TableResource);
				...
				{
					//绘画空隙
					m_TableResource.m_ImageBackSurface.BitBlt(dc,nXTablePos,nYTablePos,SizeTable.cx,SizeTable.cy,nXTablePos,nYTablePos);
				}
			}
		}
		...
		if (m_nXExcursion>rcClip.left)
		{
			...
			m_TableResource.m_ImageBackSurface.BitBlt(dc,rcClip.left,rcClip.top,nTileWidth,rcClip.Height(),rcClip.left,rcClip.top);
		}
		...
		if ((m_nYExcursion-m_nScrollPos)>rcClip.top)
		{
			...
			m_TableResource.m_ImageBackSurface.BitBlt(dc,rcClip.left,rcClip.top,rcClip.Width(),nTileHeight,rcClip.left,rcClip.top);
		}
	}
	else 
	{
...
VOID CTableViewFrame::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar * pScrollBar)
{
	...
	if (nLastPos!=m_nScrollPos)
	{
		...
		//更新视图
		RedrawWindow(&rcClient,NULL,RDW_INVALIDATE|RDW_ERASE|RDW_ERASENOW|RDW_UPDATENOW);
	}
