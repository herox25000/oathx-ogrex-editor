...
BOOL CDlgStatus::OnInitDialog()
{
	...
	//设置按钮
	m_btGiveUp.SetWindowPos(NULL,295,5,0,0,SWP_NOSIZE|SWP_NOZORDER);
...
BOOL CDlgStatus::OnEraseBkgnd(CDC * pDC)
{
	...
	//绘画进度
	ImageStatusLine.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2,35,SizeStatusLine.cx,SizeStatusLine.cy,0,0);
	ImageStatusLine.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2,35,SizeStatusLine.cx*m_uImageIndex/100L,SizeStatusLine.cy,0,SizeStatusLine.cy);

	//绘画进度
	INT nXPoint=SizeStatusLine.cx*m_uImageIndex/100L;
	ImageStatusPoint.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2+nXPoint-SizeStatusPoint.cx/2,35+(SizeStatusLine.cy-SizeStatusPoint.cy)/2);

	//文字位置
	CRect rcDescribe;
	rcDescribe.SetRect(70,16,70+184,16+18);
