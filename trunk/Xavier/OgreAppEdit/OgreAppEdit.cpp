...
namespace Ogre
{
	...
	bool		AppEdit::addEditor(BaseEditor* pEditor)
	{
		if (pEditor != NULL)
		{
			...
			{
#ifdef _OUTPUT_LOG
				TKLogEvent("已加载 编辑器:" + pEditor->getTypeName() + " OK", LML_NORMAL);
#endif
	...
	void		AppEdit::delEditor(BaseEditor* pEditor)
	{
		if (pEditor != NULL)
		{
			...
			{
#ifdef _OUTPUT_LOG
				TKLogEvent("已卸载 编辑器:" + pEditor->getTypeName() + " OK", LML_NORMAL);
#endif
	...
	void		AppEdit::clearEditor()
	{
		...
		{
#ifdef _OUTPUT_LOG
			TKLogEvent("已卸载 编辑器:" + itEitor->second->getTypeName() + " OK", LML_NORMAL);
#endif
