	...
	if (m_nState == EDIT_STATE_PAINT)
	{
		...
		if (pTerrainServer)
		{
			...
			if (pGroup)
			{
				...
				Sphere sphere(vPos, 12000.0f * 0.002);
...
void	XavierFrameContext::execturTerrain(Ogre::Terrain* terrain, const Ogre::Vector3& vCenter, float fElapsed)
{
	...
	long startx = (vPos.x - 0.002) * imgSize;
	long starty = (vPos.y - 0.002) * imgSize;
	long endx = (vPos.x + 0.002) * imgSize;
	long endy= (vPos.y + 0.002) * imgSize;
	...
	for (long y = starty; y <= endy; ++y)
	{
		for (long x = startx; x <= endx; ++x)
		{
			...
			Real weight = std::min((Real)1.0, 
				Math::Sqrt(tsYdist * tsYdist + tsXdist * tsXdist) / Real(0.5 * 0.002));
			weight = 1.0 - (weight * weight);

			float paint = weight * fElapsed;
