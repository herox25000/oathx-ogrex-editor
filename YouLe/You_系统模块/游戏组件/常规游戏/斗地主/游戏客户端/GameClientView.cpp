...
DeferWindowPos(hDwp,m_btScore,NULL,nWidth-rcScore.Width()-m_sizeHistory.cx-2,0,0,0,uFlags);
	...
	SetFlowerControlInfo(1,nWidth/2-BIG_FACE_WIDTH,nHeight-BIG_FACE_HEIGHT-m_nYBorder-5
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		//	if (i!=1)
			...
			DrawUserFace(pDC,(tagUserData *)pUserData,m_ptFace[i].x,m_ptFace[i].y,pUserData->cbUserStatus==US_OFFLINE);
		}
	}
	...
	if (m_wLandUser!=INVALID_CHAIR)
	{
		...
		if (pUserData!=NULL)
		{
			...
			//绘画头像
			DrawUserFace(pDC,(tagUserData *)pUserData,m_nXBorder+5,m_nYBorder+5,false);
...
m_ImageHistoryScore.BitBlt(pDC->m_hDC,nWidth-m_ImageHistoryScore.GetWidth()-1,1);
#else
