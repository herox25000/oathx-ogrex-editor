...
{
	m_crBoradFrame = RGB(255,255,255);
}
...
VOID CSkinComboBoxList::OnNcPaint()
{
	...
	WindowDC.Draw3dRect(0,0,rcWindow.Width(),rcWindow.Height(),m_crBoradFrame,m_crBoradFrame);
...
CSkinComboBox::CSkinComboBox()
{
	...
	m_crEnableBorad = RGB(255,255,255);
...
VOID CSkinComboBox::DrawControlView(CDC * pDC)
{
	...
	if (((GetStyle()&0x000F)==CBS_DROPDOWN)||(GetCurSel()==LB_ERR))
	{
		if (bDisable==false)
		{
			COLORREF crColor=m_crEnableBack;
	...
	if (m_bDrawBorad==true)
	{
		if (bDisable==false)
		{
			...
			//else
			//{
				COLORREF crEnableBorad = m_crEnableBorad;
				pDC->Draw3dRect(0,0,rcClient.Width(),rcClient.Height(),crEnableBorad,crEnableBorad);
			//}
		}
		else
		{
			COLORREF crDisableBorad = m_crDisableBorad;
			pDC->Draw3dRect(0,0,rcClient.Width(),rcClient.Height(),crDisableBorad,crDisableBorad);
		}
	}
	else
	{
		COLORREF crEnableBorad = m_crEnableBack;
		pDC->Draw3dRect(0,0,rcClient.Width(),rcClient.Height(),crEnableBorad,crEnableBorad);
	}
	...
	{
		ImageButton.LoadFromResource(GetModuleHandle(SKIN_CONTROL_DLL_NAME),MAKEINTRESOURCE(IDB_SKIN_COMBOBOX_BUTTON));
	}
	else
	{
	...
	//绘画按钮
	CSize SizeButton;
	SizeButton.SetSize(ImageButton.GetWidth()/m_statusNum,ImageButton.GetHeight());
...
//设置颜色
void CSkinComboBox::SetColor(COLORREF crEnableBack,COLORREF crEnableBorad,COLORREF crDisableBorad)
{
	m_crEnableBack = crEnableBack;
...
//设置资源
VOID CSkinComboBox::SetButtonImage(HINSTANCE hResInstance, UINT uResourceID, CSize & SizeButton,int statusNum)
{
	//获取位置
	CRect rcClient;
	GetClientRect(&rcClient);
