...
// CreateButton()
//
// Purpose:     The CreateButton() reate button and Loads the image for the button.  This 
...
HBRUSH CGdipButton::CtlColor(CDC* pScreenDC, UINT nCtlColor) 
{
	if(m_nStatusNum > 1)
	{
		if(!m_bHaveBitmaps)
		{
			...
			{
				float ButtonWidth = (float)m_pStdImage->m_pBitmap->GetWidth()/m_nStatusNum;
				float ButtonHeight = (float)m_pStdImage->m_pBitmap->GetHeight();

				RectF grect; 
				grect.X=0, grect.Y=0; grect.Width = ButtonWidth; grect.Height = ButtonHeight;

				PaintBk(pDC);
				graphics.DrawImage(*m_pStdImage, grect, 0, 0, ButtonWidth, ButtonHeight, UnitPixel);
				...
				{
					PaintBk(pDC);
					graphics.DrawImage(*m_pStdImage, grect, ButtonWidth, 0, ButtonWidth, ButtonHeight, UnitPixel);
				...
				{
					PaintBk(pDC);
					graphics.DrawImage(*m_pStdImage, grect, ButtonWidth*2, 0, ButtonWidth, ButtonHeight, UnitPixel);
				...
				if(m_dcGS.m_hDC == NULL)
				{
					...
					if(m_nStatusNum>3)
					{
						graphics.DrawImage(*m_pStdImage, grect, ButtonWidth*(m_nStatusNum-1), 0, ButtonWidth, ButtonHeight, UnitPixel);
					}
					else
					{
						...
						graphics.DrawImage(*m_pStdImage, grect, 0, 0, ButtonWidth, ButtonHeight, UnitPixel,&ia);
					}
