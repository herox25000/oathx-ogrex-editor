...
void CPropertyItem::OnBnClickedOk()
{
	...
	if ( lMeScore < lMePrice )
	{
		...
		{
			ShellExecute(NULL,TEXT("open"),TEXT("http://www.youle8.com"),NULL,NULL,SW_SHOWDEFAULT);
		}
...
void CPropertyItem::SetUserDataInfo(tagUserData *pUserData)
{
	...
	{
		//查找玩家
		INT nFineIndex=0,nSelItem = 0;
		while (true)
		{
			nFineIndex = m_ComboBoxTargetUser.FindString(nSelItem, pUserData->szName);
			if(nFineIndex==nSelItem || nFineIndex==CB_ERR)break;
			nSelItem = nFineIndex;
		}
