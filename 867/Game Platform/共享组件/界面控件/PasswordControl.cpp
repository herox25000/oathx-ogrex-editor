...
VOID CPasswordControl::PreSubclassWindow()
{
	...
	m_btKeyboard.SetButtonImage(IDB_BT_KEYBOARD,GetModuleHandle(SKIN_CONTROL_DLL_NAME),false,false);
...
//获取密码
LPCTSTR CPasswordControl::GetUserPassword(TCHAR szPassword[PASS_LEN])
{
	//控件密码
	if (m_bModify==true)
	{
	...
	//拷贝密码
	lstrcpyn(szPassword,m_szPassword,PASS_LEN);
...
VOID CPasswordControl::OnPaint()
{
	...
	{
		dc.FillSolidRect(rcClient,CSkinPngDialog::m_SkinAttribute.m_crBackGround);
	}
...
BOOL CPasswordKeyboard::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	...
	{
		SetCursor(LoadCursor(GetModuleHandle(0),MAKEINTRESOURCE(IDC_HAND_CUR)));
		return true;
	}
