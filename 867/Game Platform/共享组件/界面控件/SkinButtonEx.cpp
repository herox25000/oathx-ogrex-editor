...
//时间消息
VOID CSkinButtonEx::OnTimer(UINT_PTR nIDEvent)
{
	//过渡变量
...
//设置渲染
VOID CSkinButtonEx::SetRenderImage(bool bRenderImage)
{
	//设置变量
...
//设置颜色
VOID CSkinButtonEx::SetButtonColor(COLORREF crButtonText)
{
	//设置变量
...
//加载位图
bool CSkinButtonEx::SetButtonImage(UINT uBitmapID, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置变量
	m_bRenderImage=bRenderImage;
...
//加载位图
bool CSkinButtonEx::SetButtonImage(LPCTSTR pszResource, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置变量
	m_bRenderImage=bRenderImage;
	m_bTransparent=bTransparent;
...
//加载位图
bool CSkinButtonEx::SetButtonImage(UINT uBitmapID, LPCTSTR pszSurface, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置变量
	m_bRenderImage=bRenderImage;
	m_bTransparent=bTransparent;
...
//加载位图
bool CSkinButtonEx::SetButtonImage(LPCTSTR pszResource, LPCTSTR pszSurface, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置资源
	m_pszSurface=pszSurface;
	m_pszResource=pszResource;
...
//调整位置
VOID CSkinButtonEx::RectifyControl()
{
	//调整判断
	...
	else
	{
		//加载资源
		CBitImage ImageButton;
		ImageButton.LoadFromResource(AfxGetInstanceHandle(),IDB_SKIN_BUTTON);
...
//光标消息
BOOL CSkinButtonEx::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	//设置光标
	SetCursor(LoadCursor(m_hResInstance,MAKEINTRESOURCE(IDC_HAND_CUR)));

	return TRUE;
}

//销毁消息
VOID CSkinButtonEx::OnNcDestroy()
{
	//状态变量
	m_bHovering=false;
	m_bRenderImage=true;
