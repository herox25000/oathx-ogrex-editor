...
//! Tab所需资源所在的根目录 [10/11/2012 bbsssssss]
#define TAB_RES_BASE_DIR			"Resource\\Games\\"
...
bool CTableResource::LoadFromFiles(LPCTSTR pszGameName)
{
	try
	{
		...
		_snprintf(szResourcePath,sizeof(szResourcePath),TEXT("%s%s%s\\TableFrame.DAT"),g_GlobalUnits.GetWorkDirectory(), TAB_RES_BASE_DIR,pszGameName);
		...
		{
			m_bDZShowHand=true;
			_snprintf(szResourcePath,sizeof(szResourcePath),TEXT("%s%s%s\\Player.BMP")
		...
		//加载锁位图
		_snprintf(szResourcePath,sizeof(szResourcePath),TEXT("%s%s%s\\Locker.BMP")
			,g_GlobalUnits.GetWorkDirectory(), TAB_RES_BASE_DIR,pszGameName);
		...
		//加载准备标志
		_snprintf(szResourcePath,sizeof(szResourcePath),TEXT("%s%s%s\\UserReady.BMP")
			,g_GlobalUnits.GetWorkDirectory(),TAB_RES_BASE_DIR,pszGameName);
