...
#define SMALLNINE_CFG_FILENAME	"./Car.cfg"
...
BOOL CConfigDialog::OnInitDialog()
{
	...
	if (!bResult)
	{
		...
		SetDlgItemText(IDC_EDIT2,	"11031");
		SetDlgItemText(IDC_EDIT3,	"d746e3733dff946b0db30eb3fb0069c4");
		SetDlgItemText(IDC_EDIT4,	"871");
...
void CConfigDialog::OnAplyConfig()
{
	...
	if (pApp)
	{
		...
		pApp->wShunMenRate = atol(buffer.GetBuffer());

		GetDlgItemText(IDC_EDIT22, buffer);
		pApp->wTianMenRate = atol(buffer.GetBuffer());

		GetDlgItemText(IDC_EDIT23, buffer);
		pApp->wDaoMenRate = atol(buffer.GetBuffer());

		GetDlgItemText(IDC_EDIT24, buffer);
		pApp->wZuoJiaoRate = atol(buffer.GetBuffer());

		GetDlgItemText(IDC_EDIT25, buffer);
		pApp->wYouJiaoRate = atol(buffer.GetBuffer());

		GetDlgItemText(IDC_EDIT26, buffer);
		pApp->wQiaoRate = atol(buffer.GetBuffer());

		GetDlgItemText(IDC_EDIT27, buffer);
		pApp->wExtraRate1 = atol(buffer.GetBuffer());

		GetDlgItemText(IDC_EDIT28, buffer);
		pApp->wExtraRate2 = atol(buffer.GetBuffer());
	}
}
...
void CConfigDialog::OnLoadConfig()
{
	...
	sprintf(szText, "%d", pConfig->wShunMenRate);
	SetDlgItemText(IDC_EDIT21,	szText);

	sprintf(szText, "%d", pConfig->wTianMenRate);
	SetDlgItemText(IDC_EDIT22,	szText);

	sprintf(szText, "%d", pConfig->wDaoMenRate);
	SetDlgItemText(IDC_EDIT23,	szText);

	sprintf(szText, "%d", pConfig->wZuoJiaoRate);
	SetDlgItemText(IDC_EDIT24,	szText);

	sprintf(szText, "%d", pConfig->wYouJiaoRate);
	SetDlgItemText(IDC_EDIT25,	szText);

	sprintf(szText, "%d", pConfig->wQiaoRate);
	SetDlgItemText(IDC_EDIT26,	szText);

	sprintf(szText, "%d", pConfig->wExtraRate1);
	SetDlgItemText(IDC_EDIT27,	szText);

	sprintf(szText, "%d", pConfig->wExtraRate2);
	SetDlgItemText(IDC_EDIT28,	szText);
