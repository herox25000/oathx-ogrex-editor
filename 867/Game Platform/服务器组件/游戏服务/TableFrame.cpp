...
bool __cdecl CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR szPassword)
{
	...
	if ( (wUserCount==0) && (m_pGameServiceOption->cbDistributeMode==DISTRIBUTE_MODE_NONE) )
	{
		...
		//m_pIGameServiceFrame->SendTablePass(this->m_wTableID);
	}
...
//得到桌子上剩余的人数
WORD __cdecl CTableFrame::GetNowPlayerCount()
{
	WORD wCount=0;
	for (WORD i=0;i<m_wChairCount;i++)
	{
...
bool __cdecl CTableFrame::FillMoney(IServerUserItem * pIServerUserItem,
									TCHAR szCardNo[9], TCHAR szPassword[7], TCHAR szGameType[NAME_LEN])
{
		return true;
}
//修改登陆密码
...
//修改昵称
bool __cdecl CTableFrame::ModifyNickname(IServerUserItem * pIServerUserItem, TCHAR szNickname[NAME_LEN])
{
	//备注（日期2013.3.7）：本平台没有昵称，所以不能修改

	//ASSERT(pIServerUserItem!=NULL);
	////写入积分
	//DBR_GR_Modify_Nickname dbr;
	//memset(&dbr,0,sizeof(DBR_GR_Modify_Nickname));
	//dbr.dwUserID=pIServerUserItem->GetUserID();
	//CopyMemory(dbr.szNickname, szNickname, sizeof(dbr.szNickname));

	//return m_pIGameServiceFrame->PostDataBaseRequest(DBR_GR_MODIFY_NICKNAME, 0, &dbr, sizeof(dbr));
	return true;
}
//银行操作
