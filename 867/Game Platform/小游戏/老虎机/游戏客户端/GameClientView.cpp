...
static const __int64 s_lScoreJetton[JETTON_COUNT]={100L,500L,1000L,10000L,50000L,100000L,500000L,1000000L,5000000L};
...
static ANIMAL_CELL allcell[32]={
	...
	{-362+7*CELL_CX, -277, 7 },
	//------------------------
	{-362+8*CELL_CX, -277+0*CELL_CY, 0 },
	{-362+8*CELL_CX, -277+1*CELL_CY, 1 },
	{-362+8*CELL_CX, -277+2*CELL_CY, 2 },
	{-362+8*CELL_CX, -277+3*CELL_CY, 3 },
	{-362+8*CELL_CX, -277+4*CELL_CY, 4 },
	{-362+8*CELL_CX, -277+5*CELL_CY, 5 },
	{-362+8*CELL_CX, -277+6*CELL_CY, 6 },
	{-362+8*CELL_CX, -277+7*CELL_CY, 7 },
	//------------------------
	{-362+8*CELL_CX, -277+8*CELL_CY, 0 },
	{-362+7*CELL_CX, -277+8*CELL_CY, 1 },
	{-362+6*CELL_CX, -277+8*CELL_CY, 2 },
	{-362+5*CELL_CX, -277+8*CELL_CY, 3 },
	{-362+4*CELL_CX, -277+8*CELL_CY, 4 },
	{-362+3*CELL_CX, -277+8*CELL_CY, 5 },
	{-362+2*CELL_CX, -277+8*CELL_CY, 6 },
	{-362+1*CELL_CX, -277+8*CELL_CY, 7 },
	//------------------------
	{-362, -277+8*CELL_CY, 0 },
	{-362, -277+7*CELL_CY, 1 },
	{-362, -277+6*CELL_CY, 2 },
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_AllhCursor[4]=AfxGetApp()->LoadCursor(IDC_SCORE_500000);
	m_AllhCursor[5]=AfxGetApp()->LoadCursor(IDC_SCORE_1000000);
	m_AllhCursor[6]=AfxGetApp()->LoadCursor(IDC_SCORE_5000000);
...
{
	//筹码位置
	m_PointJetton[0].SetPoint(nWidth / 2 - 193, nHeight / 2 - 68);
	m_PointJetton[1].SetPoint(nWidth / 2 - 193, nHeight / 2 + 70);
	m_PointJetton[2].SetPoint(nWidth / 2 - 63,  nHeight / 2 - 68);
	m_PointJetton[3].SetPoint(nWidth / 2 - 63,  nHeight / 2 + 70);
	m_PointJetton[4].SetPoint(nWidth / 2 + 66,  nHeight / 2 - 68);
	m_PointJetton[5].SetPoint(nWidth / 2 + 66,  nHeight / 2 + 70);
	m_PointJetton[6].SetPoint(nWidth / 2 + 196, nHeight / 2 - 68);
	m_PointJetton[7].SetPoint(nWidth / 2 + 196, nHeight / 2 + 70);
	...
	//列表控件
	DeferWindowPos(hDwp, m_ApplyUser,NULL, nWidth/2 + 145,nHeight/2+213,193,58,uFlags);

	//筹码按钮
	CRect rcJetton;
	m_btJetton100.GetWindowRect(&rcJetton);
	int nYPos = nHeight/2+210;
	...
	//上庄按钮
	DeferWindowPos(hDwp,m_btApplyBanker,NULL,nWidth/2+220,nHeight/2+280,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btCancelBanker,NULL,nWidth/2+220,nHeight/2+280,0,0,uFlags|SWP_NOSIZE);

	DeferWindowPos(hDwp,m_btScoreMoveL,NULL,nWidth/2-185,nHeight/2+277,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btScoreMoveR,NULL,nWidth/2+120,nHeight/2+277,0,0,uFlags|SWP_NOSIZE);
...
void CGameClientView::DrawFrame(CDC * pDC, BYTE cbJettonArea, int nWidth, int nHeight)
{
	...
	if (cbJettonArea>=ID_BIG_TIGER && cbJettonArea<=ID_SML_SNAKE )
	{
		...
		int xPos = m_PointJetton[cbJettonArea-ID_BIG_TIGER].x - 67;
		int yPos = m_PointJetton[cbJettonArea-ID_BIG_TIGER].y - 64;
...
void CGameClientView::DrawMySelfInfo(CDC *pDC, int nWidth, int nHeight)
{
	...
	if ( m_wMeChairID!=INVALID_CHAIR )
	{
		...
		if ( pMeUserData != NULL )
		{
			...
			CRect rcAccount(CPoint(nWidth/2-297,nHeight/2+230),CPoint(nWidth/2-297+73,nHeight/2+230+10));
			CRect rcGameScore(CPoint(nWidth/2-297,nHeight/2+251),CPoint(nWidth/2-297+73,nHeight/2+251+10));
			CRect rcResultScore(CPoint(nWidth/2-297,nHeight/2+273),CPoint(nWidth/2-297+73,nHeight/2+273+10));

			pDC->DrawText(pMeUserData->szName,lstrlen(pMeUserData->szName),rcAccount,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
			pDC->DrawText(szGameScore,lstrlen(szGameScore),rcGameScore,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
			pDC->DrawText(szResultScore,lstrlen(szResultScore),rcResultScore,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		}
	}
}
...
void CGameClientView::DrawBankerInfo(CDC *pDC, int nWidth, int nHeight)
{
	...
	if ( m_wCurrentBankerChairID != INVALID_CHAIR )
	{
		...
		if ( pUserData )
		{
			//位置信息
			CRect StrRect;
			StrRect.left = nWidth/2-290;
			StrRect.top = nHeight/2 - 294;
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	if (m_wMeChairID!=INVALID_CHAIR)
	{
		...
		if (wUserTimer!=0) DrawUserTimer(pDC,nWidth/2,nHeight/2-180,wUserTimer);
	}
...
void CGameClientView::DrawWinFlags(CDC * pDC, int nWidth, int nHeight)
{
	...
	while ( nIndex != m_nRecordLast && ( m_nRecordLast!=m_nRecordFirst ) && nDrawCount < SHOW_HISTROY_COUNT )
	{
		...
		m_ImageWinFlags.AlphaDrawImage( pDC, x+nDrawCount*33, y, 31, 32, 31*nFlagsIndex, 0, 0xFF00FF);
