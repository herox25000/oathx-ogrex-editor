...
public:
	...
	COLORREF						m_crOutFrame;                       //菜单颜色
	COLORREF						m_crInFrame;                        //菜单颜色
...
class SKIN_CONTROL_CLASS CSkinMenuKernel : public CMenu
{
	//控制变量
protected:
	BOOL							m_bRemoveBorder;					//标识变量
	HHOOK							m_Messagehook;						//钩子句柄

	//静态变量
public:	
	static HWND						m_hWnd;                             //窗口句柄
	static WNDPROC					m_OldWndProc;						//窗口过程
...
public:
	//标识设置
	VOID SetRemoveBorder(HINSTANCE hInstance, BOOL bRemove);

	//静态函数
public:
	// 窗口过程
	static LRESULT WINAPI CallWndProc(INT, WPARAM, LPARAM);				
	// 窗口过程
	static LRESULT WINAPI MenuWndProc(HWND, UINT, WPARAM, LPARAM);
...
public:
	//边框控制
	inline VOID RemoveMenuBorder(HINSTANCE hInstance, BOOL bRemove=TRUE);
