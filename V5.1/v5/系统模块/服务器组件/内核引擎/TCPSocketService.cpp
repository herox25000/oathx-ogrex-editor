...
LRESULT CTCPSocketService::OnSocketNotifyRead(WPARAM wParam, LPARAM lParam)
{
	try
	{
		...
		while (m_wRecvSize >= sizeof(TCP_Head))
		{
			...
			//_sntprintf(szDescribe,CountArray(szDescribe),TEXT("CTCPScoketService::OnSocketNotifyRead 消息ID=%ld,%ld"),Command.wMainCmdID,Command.wSubCmdID);
			//CTraceService::TraceString(szDescribe,TraceLevel_Warning);

			//CTraceService::TraceString(TEXT("PP棋牌演示中心 QQ:294184930"),TraceLevel_Warning);
