...
//界面定义
#define PROPERTY_CY					55									//游戏道具
#define USER_VIEW_CY				190									//用户视图
#define MEDAL_VIEW_CY				0									//奖牌视图

//分割参数
#define INCISE_SCALE				42/100								//界面分割
...
//自定面板
VOID CGameFrameControl::SetCustomPanel(CWnd * pCustomPanel)
{
	//设置面板
	m_GameFrameUserView.SetCustomPanel(pCustomPanel);
...
VOID CGameFrameControl::TrackUserItemMenu(IClientUserItem * pIClientUserItem)
{
	...
	//去掉边框
	const CGameFrameApp * pGameFrameApp = CGameFrameApp::GetInstance();
	ASSERT(pGameFrameApp!=NULL);
	UserInfoMenu.RemoveMenuBorder(pGameFrameApp->m_hInstance,true);
...
/*	if ((bMeUserItem==false)&&(cbMeUserStatus!=US_PLAYING)&&(cbUserStatus==US_LOOKON))
	...
	{
		_sntprintf(szMenuString,CountArray(szMenuString),TEXT("金币：%I64d  胜率：%5.2f%%  逃跑率：%5.2f%%"),
		pUserInfo->lScore,fUserWinRate,fUserFleeRate);
	}
	else
	{
		_sntprintf(szMenuString,CountArray(szMenuString),TEXT("成绩：%I64d  胜率：%5.2f%%  逃跑率：%5.2f%%"),
		pUserInfo->lScore,fUserWinRate,fUserFleeRate);
	}
	UserInfoMenu.AppendMenu(IDM_NULL_COMMAND,szMenuString);
...
VOID CGameFrameControl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	INT nYPosChat=rcChatArea.bottom-54;
	CSkinButton * ChatButtonArray[]={&m_btChatButton1,&m_btChatButton2,&m_btChatButton3,&m_btChatButton4};

	//聊天控件
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-35,rcChatArea.bottom-26,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-54,rcChatArea.bottom-21,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_ChatEdit,NULL,rcChatArea.left+16,rcChatArea.bottom-16,rcChatArea.Width()-72,14,uFlags);
