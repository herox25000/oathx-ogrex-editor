...
VOID CUserListNative::PreSubclassWindow()
{
	...
	InsertColumn(m_wColumnCount++,TEXT("I D"),LVCFMT_CENTER,60,0);

	//用户成绩
	m_cbDataDescribe[m_wColumnCount]=UD_SCORE;
	if((m_wServerType&GAME_GENRE_GOLD)!=0)
		InsertColumn(m_wColumnCount++,TEXT("金币"),LVCFMT_CENTER,100,0);
	else
	...
	if (nCurrentCheck==BST_UNCHECKED)
	{
		...
		if (pGlobalUserData->cbMemberOrder==0)
		{
			...
			if (nResult==IDYES)
			{
				...
				_sntprintf(szLinkUrl,CountArray(szLinkUrl),TEXT("%sPayIndex.aspx"),szPlatformLink);
